...
* **K-Nearest Neighbors**: The nearest neighbor model predicts new points by averaging values of the k most similar observations in the reference data to the new data point, with similar observations being closer in distance than different observations.
...
* **Support Vector Machine**: This model identifies a classifying hyperplane that maximizes the margin, the distance between it and the nearest data point.
...
Interestingly `weight`, which is a variable that gives the weighting factor used for each respondent, ended up being the most important variable used to predict `income`. After that, `age`, `educ`, and `marital` were the second, third, and fourth most predictive variables. This is rather unsurprising, given the correlation we saw between these demographic variables and `income` in our exploratory data analysis. It's also worth noticing that none of the categorical arts participation variables made it into the top 10 most important predictive variables, only two numeric arts participation variables—-`nbooks` and `nfair`. This may potentially be because some categorical and numeric variables are dependent. That is, whether someone attends a play, directly correlates with the number of times that that person attended. Thus, the number of times variable may provide the same information and actually, more information. Out of all the numeric arts participation variables, it makes sense that `nbooks` in particular made it into the top 10 most important predictive variables, given its distinct relationship with `income`.
