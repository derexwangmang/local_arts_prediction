...
The overall roc-auc value is 0.715, which is similar to the random forest's performance in the cross-validated folds. While informative, this roc-auc value is an average across all classes. How did its performance change across classes?
...
The predicted classifications are on the y-axis while the true values are on the x-axis. The diagonal indicates where the two meet; in other words, the predicted value matches the true value.

It seems that for truth class `Under $10,000`, the random forest predicts the class correctly or provides a faulty prediction of `$20,000 to $29,999` for the majority of the cases. In fact, the random forest classifies the majority of its input as `$20,000 to $29,999`, `30,000 to $39,999`, and `$50,000 to $74,999`. In the context of the data provided by the EDA, this makes sense: these classes make up the highest proportion in the dataset. Although we stratified by `income`, it may be that there were not many distinguishing variables in the data. That is, none of the variables may have been good predictors for the income level, causing the random forest to essentially default to the classes with the highest proportion.

Our recipe only involved dummy-encoding income, not ensuring that it was ordinal (ordered) data. However, the confusion matrix demonstrates that the majority of the predicted values were only one or two to the left or right of the truth value. In other words, had the classes been ordered, the difference between the predicted and truth values would only be one or two classes.
...
* How is the presence of other household members affected by performance in the arts?
