...
80 of the 88 variables in our data-set contain missingness, some with a considerable amount of missingness. As can be seen in the table, over 20% of the values of variables from `bar5` down to `moremost` are `NA`. That is 18 variables with more than 20% of their values missing. The majority of the variables with missingness, however, do not have more than 20% of their values missing. We postulated that there are three reasons that this missingness occurs:
...
We decided to keep one variable that had greater than 20% missingness—`mostimp`. We made this decision because this variable represents the most important reason the participant did not attend arts events more often. This seemed like it could be an important predictor for our project, particularly if financial status was the most important reason why those with lower household incomes didn't attend arts events more often.
...
In our predictive modeling project, we will attempt to predict the survey respondent's household income, which is represented by the variable `income`. As predictors we will utilize the other variables in the dataset, which are related to arts attendence and demographic information. `income` is a categorical variable with 9 levels indicating the income bracket of the respondent's household. Before conducting an initial split on the data into a training and testing set, we wanted to visualize the outcome variable to understand its distribution and check for problems.
...
We decided to initially split the data into 70% training, and 30% testing. The imbalance between the various classes of `income` shown above prove that it would be important to stratify our initial split (and later our cross-validation) by `income`. This ensures that not level of `income` is over or under represented in the training versus the testing set. The training and testing sets are read in below. Code for how this split was actually conducted can be found in the `model_set_up.R` script inside the "models" folder inside the "output" folder. The training data will be used to build our candidate models upon, and the testing data will be used as "new data" to test the final performance of our best model after tuning. 
...
As the income bracket increases, the proportion of women with that income decreases, and the proportion of men with that income increases. This is unsurprising, given the persistence of sexism in the workforce, particularly the gender pay gap and the glass ceiling in many top-tier, high-paying jobs. 
...
The graph very clearly shows that as the income bracket increases, so do the number of white people with that as their household income. Conversely, there is a larger proportion of people of color represented in the lower income brackets than in the higher income brackets. 
...
There were a few final things that we explored in our data-set prior to modeling. Given how many people responded that they had not attended or participated in a given arts event within the past year, we thought it would be interesting to look at some of the reasons *why* this is the case. Below is the distribution of the variable `bar1`, which indicates respondents' first reason for not attending local arts events/not attending them more often.
...
It could also be interesting to see how these reasons for not attending local arts events change (or remain the same) across levels of `income`. 
...
It seems that across all income levels, lack of time is the primary reason for not attending local arts events more frequently. This is somewhat contrary to our initial hypothesis, which was that money/cost of tickets would become a more important reason for not attending arts events more frequently as the respondent's household income decreased.
...
The recipes for the last two models, the tree based models, were almost exactly the same as the above model, except for how we encoded categorical variables. For the boosted tree model, we decided to one-hot encode the categorical variables instead of simply dummy encoding. Instead of converting categorical variables with n levels into n - 1 numeric variables, one-hot encoding converts *every* level of a categorical variable into its own variable with numeric values. We did this because, unlike models with linear dependencies, tree-based models can handle having all levels of a categorical variable represented as separate variables, and we thought it would be interesting to try out this kind of encoding to see how well the model performs. That recipe is below.
...
* **Boosted Tree**: The boosted tree is an ensemble model that combines subsets of weaker trees. It builds regression trees iteratively; that is, each tree builds on the tree that came before it and minimizes the errors.
  * Parameters:
    ...
    * min_n: the minimum number of data points at the end of a branch
    * learn_rate: rate at which algorithm adapts from iteration-to-iteration
  * Grid  Used: 
    * Regular grid with 5 levels for each parameter, for a total of 125 parameter combinations
  * Package Used:
    * `xgboost`
    
<br>
  
* **Elastic Net**: This model focuses on adding regularization to the ordinary least squares model, preferring some bias over high variance. To do this, the elastic net regression adds a penalty that shrinks the slopes of the various parameters in the model. In Ridge and Lasso regression, the penalty is set whereas in the generalized elastic net, the Ridge and Lasso is combined. The major difference between Lasso and Ridge regression is that lasso has the capability to get rid of unimportant parameters completely by shrinking their coefficients to zero. An elastic net model can either be completely ridge, or completely lasso regression, or somewhere in between. This is controlled via the `mixture` parameter.
  * Parameters: 
    * penalty: represents the amount of regularization
    * mixture: proportion of L1 (Lasso) regularization
  * Grid Used: 
    * Regular grid with 7 levels for each parameter, for a total of 49 parameter combinations
  * Package Used:
    * `glmnet`
  
<br>
  
* **K-Nearest Neighbors**: The nearest neighbor model predicts new points by averaging values of the k most similar observations in the reference data to the new data point, with similar observations being closer in distance than different observations. (Note: we had flagged this model for use in an ensemble by keeping the predictions and workflow after tuning, but we did not end up using the ensemble due to incompatability issues with our outcome variable.)
  * Parameters:
    * neighbors: the number of neighbors taken into account
  * Grid Used:
    * Regular grid with 20 levels
  * Package Used:
    * `kknn`
    
<br>

* **Neural Network**: Also known as multilayer perceptrons, neural networks contain layers that each have perceptrons (also known as "hidden units") within them. The data is fed through these layers, and each layer transforms the data in some meaningful way so that the this model can classify the input accurately. Each layer is associated with a weight and a bias that controls whether or not a perceptron inside a layer will "fire" (ie release an output) when the input is fed to it. All layers start with an arbitrary weight value, and then, over various iterations (or epochs), the neural network will adjust the weights based on the error of the output from the actual values (given by a loss function). (Note: we had flagged this model for use in an ensemble by keeping the predictions and workflow after tuning, but we did not end up using the ensemble due to incompatability issues with our outcome variable.)
  * Parameters:
    * hidden_units: number of perceptrons inside a layer
    * penalty: amount of weight decay
  * Grid Used:
    * Regular grid with 5 levels for each parameter, for a total of 25 parameter combinations
  * Package Used:
    * `keras`
...
* **Support Vector Machine**: This model identifies a maximal margin classifying hyperplane (meaning it maximizes the distance between it and the nearest data points) that splits the data into the appropriate classes. (Note: we had flagged this model for use in an ensemble by keeping the predictions and workflow after tuning, but we did not end up using the ensemble due to incompatability issues with our outcome variable.)
  * Parameters:
    * cost: cost of predicting a sample within/on wrong side of margin
    * rbf_sigma: precision parameter for radial basis function
  * Grid Used:
    * Regular grid with 10 levels for each parameter, for a  total of 100 parameter combinations
  * Package Used:
    * `kernlab`
...
As you can see, the random forest model performs the best out of all the model types, producing an roc-auc value of 0.712. Furthermore, there is no overlap between the roc-auc range (mean + standard error) of the random forest and range of roc-auc for the best boosted tree, the second highest performing model. This gives us conclusive evidence that the random forest model is our best performing model, and thus the model that we will use to predict the outcome values on our test data. However, before we move on to finalizing our best random forest model workflow and applying it to the test data, let's get a sense of what the parameter values are that created the best random forest model. Below, we read in the tuned object of the random forest model from its `.rds` file, and then using the `autoplot()` function to produce a graphical representation of the performance of the random forest model across different possible parameter values.
...
Interestingly, `weight`, which is a variable that gives the weighting factor used for each respondent, ended up being the most important variable used to predict `income`. After that, `age`, `educ`, and `marital` were the second, third, and fourth most predictive variables. This is rather unsurprising, given the correlation we saw between these demographic variables and `income` in our exploratory data analysis. It's also worth noticing that none of the categorical arts participation variables made it into the top 10 most important predictive variables, only two numeric arts participation variables—-`nbooks` and `nfair`. This may potentially be because the categorical and numeric arts participation variables essentially represent the same information. In fact, the numeric variables probably give us *more* information about a respondent's engagement in the arts because they show both whether or not a person attended arts events in the last 12 months as well as the *frequency* of that person's engagement with those arts activities. Out of all the numeric arts participation variables, it makes sense that `nbooks` in particular made it into the top 10 most important predictive variables, given its distinct relationship with `income`. Finally, it was clearly a good idea to keep `mostimp` in our data-set (which represents the most important reason why a respondent did not attend more arts events) since it made it on the variable importance plot as the 8th most important variable.

## Performance on Test Data

After fitting the model to the entire training dataset with tuned parameters, we reviewed its performance on the testing dataset. The code for applying the finalized and fitted model to the testing data can be found inside our `evaluation.R` script, which is located in the `models` folder inside our `output` folder. Below, the table of predicted probabilities produced by the model after applying it to the testing data has been read in, and then is fed to the `roc_auc` function in order to evaluate the performance of the model on the testing data.
...
The overall roc-auc value is 0.715, which is similar to the random forest's performance in the cross-validated folds. This consistency is a good sign, and signifies that our model didn't overfit to the training data. While informative, this roc-auc value is an average across all classes. How did the performance of our model change across classes? To investigate this question, we can plot ROC curves for each class of our outcome variable. An ROC curve graphs sensitivity (which is the number of true positives, ie correctly classified values of the event class, divided by true positives plus false negatives, ie all members of the event class) by one minus specificity (which is the true negatives, ie all observations correctly classified as not the event class, divided by the true negatives plus false positives, ie all members of the non-event classes) at different threshold values. A threshold is the decision point at which observations are categorized into certain classes of the outcome variable. For each observation in a new-data set, every model produces probability values for each class of the outcome variable indicating how confident the model is that the observation belongs in that class. The class that produced a probability above the threshold is then the class into which the observation is categorized. ROC curves represent better model performance if they are closer to a right angle. An ROC curve that is close to, or exactly at, a straight, diagonal line indicates that the model performed no better at distinguishing between the given class and any other class of the outcome than if it had done so by random chance alone. 
...
The predicted classifications are on the y-axis while the true values are on the x-axis. The x-axis starts at `Under $10,000` and goes to `$100,000  or more`, while the y-axis starts at `$100,000  or more` and goes up to `Under $10,000`. The diagonal indicates where the two meet; in other words, where the predicted value matches the true value.

It seems that the random forest model accurately predicts a lot of the values in the `Under $10,000`, `$20,000 to $29,999`, and `$50,000 to $74,999` classes (which you can see because these boxes are more shaded in more fully along the "accuracy" diagonal). For the rest of the classes, the random forest model does not do a very good job of correctly classifying the values. In fact, the random forest classifies the majority of its input as `$20,000 to $29,999`, `30,000 to $39,999`, and `$50,000 to $74,999`, which you can see by the way these rows are very full and more shaded in. In the context of the data provided by the EDA, this makes sense: `$20,000 to $29,999`, `30,000 to $39,999`, and `$50,000 to $74,999` were the classes that had the highest proportion of observations in the dataset. Although we stratified both our initial split and our folds by `income`, it may be that there were not many distinguishing variables in the data. That is, none of the variables may have been good predictors for the income level, causing the random forest to essentially default to categorizing observations into the classes with the most values in the training data.

## Conclusion

The random forest performed decently on the training and testing data. Given their empirical nature and affinity for high-dimensional data, this random forest model performed the best of the six models we tested. We would be hesitant, however, to endorse its use beyond this project, given that it can only provide accurate predictions for specific classes. There are multiple classes of the outcome in which the random forest model did a very poor job of correctly differentiating between that class and the other classes of the outcome. Attendance of the arts is unfortunately almost certainly *not* a great predictor of income. While having more money can increase one's ability to access the arts, participation in arts activities tends to only be voluntary and self-selective, fluctuating with individual interests, age, lifestyle, and responsibilities.

## Next Steps

To provide better predictions, it could be invaluable to re-examine the original training dataset, reviewing the dataset with greater domain knowledge to identify variables that we may have discarded that may actually be useful. Another way to improve performance would be to review a correlation plot and only include variables strongly correlated with the outcome variable. That way, inferential statistics could be applied to improve upon our predictive statistical analysis. Furthermore, other questions regarding the variables in this data-set to explore in the future are included here:

* How can the age of the participant be predicted by other variables within the dataset?
* Is participation in various arts activities more indicative of a respondent's marital status or age than of their income? Ie would `marital` or `age` be better outcome variables than `income` in terms of model performance?
